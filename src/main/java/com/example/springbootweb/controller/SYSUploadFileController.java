...
import com.example.springbootweb.util.FilesUUtil;
...
public class SYSUploadFileController {
    ...
    @PostMapping
    @ResponseBody
    public LHResult upload(@RequestParam("file") MultipartFile uploadFile, HttpServletRequest request)
            throws Exception {
       rm=FilesUUtil.upload(uploadFile,request);
       Integer code= (Integer) rm.get("code");
       if (code.equals(0)){
           return LHResult.succ("上传文件成功",rm);
       }else if (code.equals(1)){
           return LHResult.others("上传文件类型不符",rm);
       }
        System.out.println("传入文件：\t" + uploadFile);

        return LHResult.fail("系统错误",rm);
    }
